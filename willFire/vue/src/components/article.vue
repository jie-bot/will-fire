    ...
    methods: {
      ...
      subscribeLabel() {
        ...
        this.$confirm('确认' + (this.subscribe ? '取消订阅' : '订阅') + '专栏【' + this.article.label.labelName + '】？' + (this.subscribe ? "" : "订阅专栏后，该专栏发布新文章将通过邮件通知订阅用户。"), this.subscribe ? "取消订阅" : "文章订阅", {
          ...
          this.$http.get("http://47.96.86.223:8081/api" + "/user/subscribe", {
      ...
      submitWeiYan(content) {
        ...
        this.$http.post("http://47.96.86.223:8081/api" + "/weiYan/saveNews", weiYan)
          .then((res) => {
      ...
      getNews() {
        this.$http.post("http://47.96.86.223:8081/api" + "/weiYan/listNews", {
          current: 1,
          size: 9999,
          source: this.article.id
        })
      ...
      getArticle(password) {
        this.$http.get("http://47.96.86.223:8081/api" + "/article/getArticleById", {id: this.id, password: password})
          .then((res) => {
            if (!this.$common.isEmpty(res.data)) {
