...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getArticles() {
        await this.$http.post("http://47.96.86.223:8081/api" + "/article/listArticle", this.pagination)
          .then((res) => {
      ...
      getSortArticles() {
        this.$http.get("http://47.96.86.223:8081/api" + "/article/listSortArticle")
          .then((res) => {
            if (!this.$common.isEmpty(res.data)) {
