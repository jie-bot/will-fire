    ...
    methods: {
      handleDelete(item) {
        this.$confirm('确认删除资源？', '提示', {
          ...
          this.$http.post("http://47.96.86.223:8081/api" + "/resource/deleteResource", {path: item.path}, true, false)
            .then((res) => {
      ...
      getResources() {
        this.$http.post("http://47.96.86.223:8081/api" + "/resource/listResource", this.pagination, true)
          .then((res) => {
      ...
      changeStatus(item) {
        this.$http.get("http://47.96.86.223:8081/api" + "/resource/changeResourceStatus", {
          id: item.id,
          flag: item.status
        }, true)
          .then((res) => {
