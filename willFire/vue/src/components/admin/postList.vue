  ...
  export default {
    ...
    methods: {
      ...
      getArticles() {
        ...
        this.$http.post("http://47.96.86.223:8081/api" + url, this.pagination, true)
          .then((res) => {
            if (!this.$common.isEmpty(res.data)) {
      ...
      changeStatus(article, flag) {
        ...
        this.$http.get("http://47.96.86.223:8081/api" + "/admin/article/changeArticleStatus", param, true)
          .then((res) => {
            if (flag === 1) {
              this.$message({
                duration: 0,
      ...
      handleDelete(item) {
        this.$confirm('确认删除？', '提示', {
          ...
          this.$http.get("http://47.96.86.223:8081/api" + "/article/deleteArticle", {id: item.id}, true)
            .then((res) => {
              this.pagination.current = 1;
              this.getArticles();
              this.$message({
