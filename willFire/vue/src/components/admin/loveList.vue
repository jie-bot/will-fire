    ...
    methods: {
      handleDelete(item) {
        this.$confirm('确认删除资源？', '提示', {
          ...
          this.$http.get("http://47.96.86.223:8081/api" + "/family/deleteFamily", {id: item.id}, true)
            .then((res) => {
      ...
      getLoves() {
        this.$http.post("http://47.96.86.223:8081/api" + "/family/listFamily", this.pagination, true)
          .then((res) => {
      ...
      changeStatus(item) {
        this.$http.get("http://47.96.86.223:8081/api" + "/family/changeLoveStatus", {
          id: item.id,
          flag: item.status
        }, true)
          .then((res) => {
