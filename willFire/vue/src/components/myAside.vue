  ...
  <div>
    <div class="myAside-container">
      ...
      <div class="shadow-box-mini background-opacity wow admire-box"
           v-if="!$common.isEmpty(barrageList)">
        <div style="font-weight: bold;margin-bottom: 20px">💬 最新留言</div>
        <div>
          <vue-seamless-scroll :data="barrageList" style="height: 200px;overflow: hidden">
            <div v-for="(item, i) in barrageList"
    ...
    <!-- <el-dialog title="赞赏"
      ...
      <div>
        ...
        <div>
          <div class="admire-content">当赞赏如春风轻拂面</div>
          <div class="admire-content">心如花开，亦随风舞翩</div>
        </div>
      </div>
    </el-dialog> -->
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getTreeHole() {
        this.$http.get("http://47.96.86.223:8081/api" + "/webInfo/listTreeHole")
          .then((res) => {
      ...
      getAdmire() {
        this.$http.get("http://47.96.86.223:8081/api" + "/webInfo/getAdmire")
          .then((res) => {
            if (!this.$common.isEmpty(res.data)) {
